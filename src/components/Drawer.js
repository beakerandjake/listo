...
function DefaultCloseButton({ onClick, anchor, icon, title }) {
  const anchorStyle =
    CLOSE_BUTTON_ANCHOR_STYLES.find((x) => x.anchor === anchor)?.className ||
    invalidProp(`Unsupported anchor for close button: '${anchor}'`);
...
export function Drawer(
  ...
  const anchorStyle =
    ANCHOR_STYLES.find((x) => x.anchor === anchor)?.className ||
    invalidProp(`Unsupported anchor: '${anchor}'`);
  const transitionStyles =
    TRANSITION_STYLES.find((x) => x.anchor === anchor)?.styles ||
    invalidProp(`Transitions not supported for anchor: '${anchor}'`);
  const sizeStyle =
    SIZE_STYLES.find((x) => x.size === size && x.anchors.includes(anchor))
      ?.className || '';
      ...
      <div className="overflow-hidden">
        <Transition
          ...
          classNames={{
            ...
            appearActive: [
              'transform-[opacity,transition] ease-out duration-300',
              transitionStyles.appearActive,
            ].join(' '),
            exit: transitionStyles.exit,
            exitActive: [
              'transform-[opacity,transition] ease-in duration-200',
        ...
        >
          ...
          >
            ...
            {children}
            {!!showCloseButton && (
              <DefaultCloseButton {...defaultCloseButtonProps} />
            )}
          </DialogContent>
        </Transition>
      </div>
