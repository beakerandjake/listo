...
const MenuItemIcon = ({ icon, variant }) => {
  ...
  const variantStyle =
    VARIANT_STYLES.icon[variant] || VARIANT_STYLES.icon.default;
...
export const MenuItem = forwardRef(
  ({ icon, label, children, variant, className, disabled, ...props }, ref) => {
    return (
      <button
        {...props}
        role="menuitem"
      ...
      >
        ...
        <MenuItemLabel className="flex-1" variant={variant}>
          {label}
        </MenuItemLabel>
        {children}
      </button>
