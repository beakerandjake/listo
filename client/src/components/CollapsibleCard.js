 ...
 * @param {object} props
 * @param {string} props.title - The title of the card.
 * @param {boolean} props.defaultOpen - Should the card default to being opened?
...
export const CollapsibleCard = ({ title, defaultOpen = false, children }) => {
  return (
    <Disclosure
      ...
      className="overflow-hidden rounded-lg bg-white shadow border border-gray-300 divide-y divide-y-300"
    >
      {({ open }) => (
        <>
          <Disclosure.Button
