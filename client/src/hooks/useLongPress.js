...
export function useLongPress(pressStartTime, callback) {
  ...
  useEffect(() => {
    ...
    const holdInterval = setInterval(() => {
      ...
      if (
        callbackSpeedIndex < CALLBACK_SPEEDS.length - 1 &&
        differenceInMilliseconds(new Date(), holdStartTime) >
          CALLBACK_SPEEDS[callbackSpeedIndex].minDuration
      ) {
