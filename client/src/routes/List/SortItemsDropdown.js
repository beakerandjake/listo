...
const getSortingField = ({ itemKey = '', direction = '' } = {}) =>
  SORTING_FIELDS.find(
    (x) => itemKey === x.itemKey && direction === x.sortingDirection
  );
 ...
 */
export function SortItemsDropdown({ activeSort, onChange }) {
  const [activeSortingField, setActiveSortingField] = useState(
    getSortingField(activeSort)
  );
      ...
      {({ open, setOpen }) => (
        <ResponsiveMenu
          ...
          trigger={
            <div>
              <SwitchTransition
                switchKey={activeSortingField?.label || ''}
                as={FadeAndPopIn}
              >
                <Button
                  border="none"
